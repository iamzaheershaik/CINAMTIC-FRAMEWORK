...
export class AppComponent {
  ...
  activeFramework = signal<'cinematic' | 'articulated' | 'photoreal' | 'pro-guide'>('cinematic');
  ...
  // New signals for video generation
  isGeneratingVideo = signal(false);
  videoGenerationError = signal<string | null>(null);
  videoGenerationProgress = signal<string | null>(null);
  ...
  selectFramework(framework: 'cinematic' | 'articulated' | 'photoreal' | 'pro-guide'): void {
  ...
  private clearWorkspace(): void {
    ...
    this.videoGenerationError.set(null);
    this.videoGenerationProgress.set(null);
  ...
  async onGenerate(): Promise<void> {
    ...
    try {
      let promptGenerator: Promise<string>;
      ...
      switch (this.activeFramework()) {
        case 'cinematic':
          promptGenerator = this.geminiService.generateCinematicPrompt(subject, outputType, cameraShots);
          break;
        case 'articulated':
          promptGenerator = this.geminiService.generateArticulatedPrompt(subject, outputType, cameraShots);
          break;
        case 'photoreal':
          promptGenerator = this.geminiService.generatePhotorealPrompt(subject, outputType, cameraShots);
          break;
      }
  ...
  async onGenerateImage(): Promise<void> {
    const prompt = this.promptSections().map(s => s.content).join(' ');
  ...
  async onGenerateVideo(): Promise<void> {
    const prompt = this.promptSections().map(s => s.content).join(' ');
    if (!prompt || this.isGeneratingVideo()) return;
    ...
    try {
      const videoUrl = await this.geminiService.generateVideo(
        prompt, 
        (progressMessage) => this.videoGenerationProgress.set(progressMessage)
      );
      this.generatedVideoUrl.set(this.sanitizer.bypassSecurityTrustUrl(videoUrl));
    } catch (e) {
      this.videoGenerationError.set('Failed to generate video. Please try again.');
      console.error(e);
    } finally {
  ...
  copyToClipboard(): void {
    const textToCopy = this.outputViewMode() === 'text' 
      ? this.promptSections().map(s => `${s.title}:\n${s.content}`).join('\n\n')
      : this.jsonPrompt();
